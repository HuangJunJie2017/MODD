function [center,output_number]=fast_cluster_analysis(point,similarity_para_flow,similarity_para_coor,~,min_distanceflow,min_distancecoor,max_analyze_number)
...
if N>max_analyze_number
    ...
    point=[point(idx,:),idx];
%     min_desity=(max_analyze_number/N)*max_analyze_number/10;
else
%     min_desity=max_analyze_number/10;
    point=[point,(1:N)'];
end
...
% img_out=zeros(ww,hh);
% for i=1:N
%     img_out(point(i,3)/3,point(i,4)/3)=density(i)/maxd;%floor(density(i));%/maxd*255);
% end
% figure(21)
% img_temp=ones(ww,hh);
% [xx,yy]=find(img_temp);
...
%     img_out(point(i,3)/3,point(i,4)/3,:)=map(idx,:)*255;
% end
% figure(22)
% imshow(uint8(img_out));
